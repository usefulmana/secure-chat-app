...
router.get('/:id', passport.authenticate("jwt", { session: false }), async (req, res) => {
    ...
    const channel = await Channel.findById(cId).populate({
        path: 'members',
...
router.post('/addMember', passport.authenticate("jwt", { session: false }), async (req, res) => {
        ...
        if (String(c.server.owner) === String(req.user.id)){
            for (let i = 0; i < c.members.length; i++) {
                if (String(c.members[i]) === userId){
                    return res.status(400).send({message: "User already in this channel"})
                }
            }
            ...
            return res.status(400).send({message: "You do not have permission to do that"})
        }
