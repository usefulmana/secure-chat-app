...
import './Profile.scss'
import { changeUsername, changePassword, currentUser, changeAvatar } from '../API/userAPI'
...
const Profile = ({ history }) => {
  ...
  useEffect(() => {
    currentUser({ token }).then((data) => {
      setUser({ username: data.username, image: data.image, password: "123123123" })
    }).catch((err) => {
      console.log("err : ", err)
    })
  }, [])
  ...
  const handleSubmit = (option) => (e) => {
    if (option === "username") {
      ...
      changePassword({ password: user.password, token }).then((data) => {
      ...
      changeAvatar({ formData, token }).then((data) => {
        if (data.image) {
          setPopUp(true)
  ...
  return (
    <div className="profile-cont">
      <div><img className="avatar-image" src={`${user.image}`} /></div>
      <input type="file" name="filefield" onChange={handleChange("image")} required></input>
      ...
      <div>{user.email}</div>
      <div>
        <input type="password" onChange={handleChange("password")} value={user.password} disabled={disabled.password} />
